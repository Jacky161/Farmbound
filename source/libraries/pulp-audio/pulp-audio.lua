...
-- GENERATED FROM PulpRuntime/Source/runtime.lua
-- BY PulpRuntime/make/pulp-audio.php
...
local function setVolume(voice, v)
	...
	voice.synths[2]:setVolume(v * baseVolume)
end
local function setEnvelope(voice, a,d,s,r,v)
...
local _env = {}
local function applyEnvelope(voice, envelope)
...
function startSong(value, once, callback)
	...
	for i=1,#voices do
		applyEnvelope(voices[i], song.voices and song.voices[i])
	end
...
local function loadAudio()
	...
	for i=1,#data.sounds do
		local sound = data.sounds[i];
		if not sound then goto continue end
	...
	for i=1,#data.songs do
		local song = data.songs[i];
		if not song then goto continue end
		songsByName[song.name] = song
		if song.splits then song.splits = nil end -- unneeded
...
pulp = {
	audio = {
		kPlayLoop = false,
		kPlayOnce = true,
		init = function(songsPath,soundsPath)
			initAudio()
			data = {
				songs = json.decodeFile(songsPath or 'pulp-songs.json'),
				sounds = json.decodeFile(soundsPath or 'pulp-sounds.json'),
			}
			loadAudio()
		end,
		update = updateAudio,
